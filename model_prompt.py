...
class CLIP_Model(nn.Module):
    ...
    def forward(self, image, labels=None, clip_loss=True, cls_loss=False):
        ...
        logits_per_image_59_cls, logits_per_image_cls = None, None, None, None
        ...
        if clip_loss and self.training:
            ...
            prompts_clip = torch.stack(prompts_list)
            tokenized_prompts_clip = torch.stack(tokenized_prompts_list)
            text_features_clip = self.text_encoder(prompts_clip, tokenized_prompts_clip)
            ...
            # 有5/9类，做class损失
            if class_image_list:
                prompts_cls = torch.stack([prompts[5], prompts[9]])
                tokenized_prompts_cls = torch.stack([tokenized_prompts[5], tokenized_prompts[9]])
                image_features_cls = torch.stack(class_image_list)
                text_features_59_cls = self.text_encoder(prompts_cls, tokenized_prompts_cls)
                text_features_59_cls = text_features_59_cls / text_features_59_cls.norm(dim=1, keepdim=True)
                logits_per_image_59_cls = logit_scale * image_features_cls @ text_features_59_cls.t()
