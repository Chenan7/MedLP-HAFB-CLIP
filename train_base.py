...
parser.add_argument('--print_freq', default=50, type=int, help='print freq.')
# 损失参数
...
labels = [index for index in range(10)]
...
def main():
    ...
    # 创建文件夹
    os.makedirs(os.path.join(args.save_path, 'confusion_matrix'), exist_ok=True)
    ...
    for epoch in range(args.epochs):
        # print('before train ---Val---')
        # val_acc = validate(val_loader, model, -1)
        # print(' * Acc {acc:.4f} '.format(acc=val_acc))
        # print('before train ---Val end---')
        train(train_loader, model, ce_loss, optimizer, epoch)
        print('Val stage')
        start_time = time.time()
        val_acc = validate(val_loader, model, epoch)
        print(' * Acc {acc:.4f} '.format(acc=val_acc))
        ...
        if is_best:
            ...
            print('Test stage')
            test_acc = validate(test_loader, model, epoch)
...
def train(train_loader, model, ce_loss, optimizer, epoch):
    ...
    for i, (image, label) in enumerate(train_loader):
        ...
        logits_per_image_clip, logits_per_text_clip, logits_per_image_cls = model(image, label, args.clip_loss, args.cls_loss)
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  .format(
                   ...
                   data_time=data_time, loss=losses, loss_clip=losses_clip, loss_cls=losses_cls))
